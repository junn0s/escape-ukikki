#!/usr/bin/env python3
"""Prepare transparent RX-9 quarantine support props and a QA sheet."""
...
ASSET_SPECS = {
    "S_ContainmentLock.png": ((512, 256), (18, 14)),
    "S_ObservationConsole.png": ((384, 384), (18, 18)),
    "S_DeconShower.png": ((384, 384), (18, 18)),
    "S_RestraintRail.png": ((1024, 96), (24, 8)),
    "S_RestraintController.png": ((256, 384), (14, 18)),
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--lock-source", type=Path, required=True)
    parser.add_argument("--console-source", type=Path, required=True)
    parser.add_argument("--shower-source", type=Path, required=True)
    parser.add_argument("--rail-source", type=Path, required=True)
...
def build_background(
    ...
    canvas = Image.new("RGBA", (1600, 950), (17, 24, 32, 255))
    if floor_tile_path is not None and floor_tile_path.is_file():
        ...
        for y in range(320, canvas.height, tile.height):
    ...
    if wall_tile_path is not None and wall_tile_path.is_file():
        with Image.open(wall_tile_path) as source:
            wall = source.convert("RGB").resize(
                (512, 320), Image.Resampling.LANCZOS
            )
...
def make_preview(
    ...
    placements = {
        "S_ContainmentLock.png": ((70, 95), (560, 280)),
        "S_ObservationConsole.png": ((700, 35), (360, 360)),
        "S_DeconShower.png": ((1170, 35), (340, 340)),
        "S_RestraintRail.png": ((70, 500), (1050, 100)),
        "S_RestraintController.png": ((1240, 490), (230, 345)),
    }
    draw_shadow(canvas, (40, 65, 660, 405), 48)
    draw_shadow(canvas, (670, 10, 1090, 420), 52)
    draw_shadow(canvas, (1140, 10, 1540, 405), 52)
    draw_shadow(canvas, (40, 465, 1150, 635), 44)
    draw_shadow(canvas, (1205, 455, 1505, 865), 48)
...
def main() -> None:
    args = parse_args()
    sources = {
        "S_ContainmentLock.png": args.lock_source,
        "S_ObservationConsole.png": args.console_source,
        "S_DeconShower.png": args.shower_source,
        "S_RestraintRail.png": args.rail_source,
        "S_RestraintController.png": args.controller_source,
    }
    sprites = {
