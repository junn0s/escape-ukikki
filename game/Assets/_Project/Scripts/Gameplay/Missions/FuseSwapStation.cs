...
{
    /// <summary>
    /// 전력 복구실의 퓨즈 교체 미션이다(GDD §10.2). 탄 퓨즈를 클릭해 뽑고
    /// 새 퓨즈를 드래그해 꽂는다. 공기 필터 교체와 같은 2단계 판정을
    /// 공유한다. 실제 상태 전이는 서버가 판정하고 이 컴포넌트는 표시와
    /// 요청만 담당한다.
    /// </summary>
    public sealed class FuseSwapStation : MonoBehaviour, IInteractable
    {
        ...
        public event Action<FuseSwapStation> StateChanged;
        public event Action<FuseSwapStation, GameObject> MissionOpened;
        ...
        /// <summary>퓨즈를 뽑을 때는 false, 새 퓨즈를 꽂을 때는 true로 요청한다.</summary>
        ...
        public void ApplyAuthoritativeState(
            bool isOldFuseRemoved,
            bool isNewFuseInstalled)
        {
            Rules.Reset();
            if (isOldFuseRemoved)
            {
