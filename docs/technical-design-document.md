...
`MixedPerspectiveSceneStyler`는 기존 씬의 아래쪽 벽 정면을 2.45m로 확장하고 방 바닥 경계와
벽 구간의 교차를 계산해 방별 벽지 패턴과 벽걸이 시스템 패널을 런타임에 멱등 생성한다.
벽 폭이 충분할 때만 CCTV·전자시계·상태계를 하나 더 만들며 반복 공용 벽 슬롯은 숨긴다.
기존 씬의 `Room_{RoomId}` 렌더러에는 `Resources/Environment/Floors/T_Floor_{RoomId}`를
로드해 4m 단위로 반복하고 색상을 흰색으로 초기화한다. 빌더도 같은 경로를 우선하므로 현재 씬과
재생성 씬의 방별 바닥이 일치하며, 전용 텍스처가 없을 때만 공용 회색 타일·색조로 폴백한다.
...
장애물의 첫 충돌 지점까지 3단 Freeform `Light2D` 경로를 갱신한다. 근거리·중거리·원거리 광원을
겹쳐 기존 Point Light의 거리 감쇠를 근사하므로 Unlit 원뿔이 화면을 덮지 않고 Lit 바닥과 프롭을
실제로 밝힌다. 플레이어·괴물과 Trigger는 차폐 대상에서 제외하며, 문 충돌체가 비활성화된 열린
출입구로는 빛이 통과한다. 따라서 직교 카메라와 물리 좌표를 바꾸지 않고도 벽 뒤 방 바닥에
손전등이 투영되지 않는다.
...
생존자 22종은 `SurvivorMissionCatalog`의 고정 ID를 사용하며, 씬 재생성이나
`NetworkObjectId` 변화가 개인 일지·재접속 스냅샷의 미션 정체성을 바꾸지 않는다.
`SurvivorTeamMissionAssignmentService`는 생존자 전체를 한 번에 배정해 라운드 안에서 스테이션이
중복되지 않도록 하고, 시작 위치·난이도·미니게임 종류를 기준으로 각자 4~5개를 선택한다.
...
구형 `MissionStation_` 직렬화가 남았거나 백신·실험실·격리실·입원실·액체 보관실·보안실·
전력실 미션에
최종 장비 아트가 없는 저장소를 열면
에디터 마이그레이션이 미저장 씬이 없는 경우에만 새 빌더 배치로 한 번 재생성하고, 현재
미션 배치와 장비 아트가 모두 있는 씬에서는 아무 작업도 하지 않는다.
클라이언트는 성공 bool 대신 드래그·타이밍·회전·버튼·문지르기 입력을 보내고, 각 미션의
서버 권위 컴포넌트가 순서·짝·허용 구간을 다시 계산한 뒤 공통 권위 계층을 통해서만 개인
완료와 프로젝트 점수를 확정한다.

최종 멀티 사전 검사는 `Validate Final Multiplayer Setup` 메뉴에서 완성 미션 장비·외곽선·
구형 플레이스홀더 비활성화, 4개 필수 빌드 씬, HostClient2의 Main Editor와 Player 2가
부트스트랩 씬을 사용하는지를 함께 확인한다. 이 검사는 구조 검증이며 실제 Relay 접속 완료는
HostClient2 실행 뒤 `Test Host Client Relay`를 통과해야 확정한다.
