...
- `main`: 시연 가능한 안정 상태이자 기본 작업 대상
- `feature/<short-name>`, `fix/<short-name>`: 여러 명이 같은 파일을 동시에 건드릴 때만 사용

해커톤 기간이 짧아 실제 작업은 `main`에 직접 커밋하는 방식으로 진행한다. 초기에는 PR을 사용했지만
1인 작업 구간에서는 브랜치를 나누지 않는다. 씬과 프리팹은 병합이 어려우므로, 같은 씬을 두 명이
동시에 수정해야 할 때만 브랜치를 파고 되도록 빨리 `main`에 합친다.

해커톤 기간이 짧아도 작업 단위 커밋을 유지한다. 씬과 ProjectSettings 변경은 커밋 메시지에 명시한다.

### 10.5 커밋 메시지

한글 접두사를 사용하고, 제목은 명사형으로 끝낸다.
...
| `수정:` | 버그 수정 |
| `문서:` | 문서만 변경 |
| `정리:` | 리팩터링, 설정, 빌드 등 동작 변화가 없는 변경 |

```text
기능: 빌런 강화 3축 구현

후각·개체·독성 강화를 서버 권위로 판정하고 축마다 0~2 단계로 보관한다.

- 실험실 B·백신실 B·입원실에 축별 강화 스테이션 배치
- 강화 단계는 빌런 소유 클라이언트에만 전달
```

- 제목은 50자 안쪽으로 쓰고, 상세 내용은 본문에 목록으로 남긴다.
- 커밋 메시지에 AI 협업 흔적(`Claude`, `Generated with`, `Co-authored-by` 등)을 남기지 않는다.
