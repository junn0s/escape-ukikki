...
캐릭터는 3등신 데포르메지만 실제 높이는 약 1.6m다(`art-audio-asset-guide.md` §1.4). 위 공간
수치는 그대로 쓴다. 다만 머리가 커서 문과 좁은 복도에서 카메라 가림과 다른 플레이어 식별이
나빠질 수 있으므로, 그레이박스 검증 시 문 폭 1.6~1.8m를 우선 확인한다.
...
- 플레이어가 가려지는 기둥은 카메라 투명화 대상에 포함한다.
...
- 모든 주요 방에 한 개씩
- 긴 복도 교차점에 최대 두 개 추가
- 스피커 위치는 천장 또는 높은 벽이지만 LED는 탑뷰에서 보이게 한다.
...
- 기본 각도는 수평 기준 50~60도 하향이다.
- 카메라 회전은 제공하지 않는다.
- 플레이어와 카메라 사이의 천장, 전면 벽, 큰 배관을 투명화한다.
- 뒤쪽 벽은 방의 깊이감을 위해 유지한다.
- 투명화 전환은 0.15~0.3초 페이드한다.
- 방 진입 시 천장 그룹 단위로 숨긴다.
- CCTV 카메라는 고정 시점이며 플레이어 카메라와 별도 렌더 텍스처를 사용한다.

---

## 11. NavMesh와 충돌

- 플레이어와 괴물은 같은 이동 가능 바닥을 사용하되 에이전트 반경을 다르게 둔다.
- 가구 아래와 장식 틈은 NavMesh에서 제외한다.
- 자동문은 열림 상태에서 경로가 연결되도록 한다.
- 문 충돌은 서버 판정과 클라이언트 표시가 일치해야 한다.
- 좁은 문 앞에 괴물 대기 지점을 여러 개 둬 겹침을 줄인다.
- 추가 괴물 생성 위치는 NavMesh 중심에서 충분히 떨어진 개별 지점으로 둔다.
