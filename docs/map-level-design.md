...
한 오브젝트가 여러 미션 변형을 제공할 수 있다. 정상 미션 패널과 빌런 위장 미션 프록시는
같은 좌표에 겹쳐 배치하고 역할별 서버 판정으로 서로 다른 퍼즐을 연다. 두 변형의 점유는
독립적이므로 생존자와 빌런이 같은 패널을 동시에 조작할 수 있으며, 외형만으로는 구분할 수 없다.
실험실 B의 후각 패널은 밸브 드래그, 보안실의 개체 패널은 회로 클릭, 백신실 B의 독성 패널은
타이밍 주입을 사용해 빌런 강화 3종도 서로 다른 실제 조작을 제공한다.

현재 회색상자에서는 10개 방에 다음 9가지 조작을 분산한다.

| 방 | 미션 | 주 조작 |
| --- | --- | --- |
| 백신실 A | 시료 분류 | 시료·보관함 클릭 |
| 실험실 A | 서버 로그 복구 | `Q/W/E/R/T` 순서 입력 |
| 격리실 A | 출입문 보안 회로 | 모듈 클릭 회전 + `Space` |
| 액체 보관실 | 비상 배터리 운반 | 드래그 분리 + 월드 운반 + 장착 |
| 보안실 | CCTV 재부팅 | 신호·포트 클릭 연결 |
| 전력 복구실 | 퓨즈 복구 | 퓨즈 드래그 |
| 입원실 | 구조 안테나 조율 | 방향키/WASD + `Enter` |
| 실험실 B | 시료 분류 | 시료·보관함 클릭 |
| 격리실 B | 압력 안정화 | 원형 밸브 드래그 + 잠금 레버 |
| 백신실 B | 차단기 기동 | 타이밍 클릭 또는 `Space` |

액체 보관실에서 분리한 비상 배터리의 수신 단말기는 입원실에 둔다.
