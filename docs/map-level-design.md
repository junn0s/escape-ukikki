...
- 표현: 낡은 CRT·수리 테이프·스테인리스 폐기함을 사용한 제약회사 데이터·오염 폐기 구역
...
- 표현: 깨진 법랑 세척대·노출된 배관·원색 시약병·글러브 포트 배양통을 사용한 노후
  병리·습식 화학실
...
- 표현: 낡은 법랑 배선함·올리브색 아날로그 압력 제어기·적색 제독 버튼을 사용한 군용
  방공호 개조형 화학 제독 구역
...
- 표현: 산화된 청록 배관·대형 적색 핸드휠·황동 압력계와 유압식 폐기물 드럼 압축기를
  사용한 제약회사 지하 배관·폐기 구역
...
- 표현: 낡은 기관용 카드 리더, 오염된 이중 CRT 감시대, 케이블 패치 패널을 사용한
  1980년대 아날로그 감시센터
...
- 표현: 도자기 절연체가 달린 고압 차단기, 개방형 퓨즈함, 노출 구리 전력선을 사용한
  오래된 도시 변전실
...
- 장비 표현: 황변한 베이지 CRT 바이탈 모니터·기계식 수액 타이밍 장치·파란 투약 차트
  파쇄기를 사용한 1990년대 야전병원형 병동
...
- 표현: 빈티지 광학 현미경·유리 적정 장치·금속 동물 케이지를 사용한 대학 생물학·동물
  실험실
...
- 단서: 두 번째 개체 강화 흔적
- 표현: 오염·신품 필터가 함께 노출된 캐비닛과 녹슨 대형 환풍기·수동 역류 레버를 사용한
  화학 공장 HVAC 정비 구역
...
- 표현: 결빙된 냉동고와 흰색·민트색 임상 스캐너를 사용한 백신 냉동 유통·검사 구역
...
- 미션 동선을 침범하던 대형 가구를 제외하고 방별 전용 교체 슬롯 87개를 배치한다.
  제작 설비, 의료 장비, 서버·보안 장비,
  격리 설비, 전력 장비와 환경 서사 데칼이 포함된다.
- 방마다 공용 벽 설비 4개, 벽체 마감·코너 보호대·중앙 통행선 10개를 추가해
  복도와 자동문을 제외하고 총 227개의 방 내부 교체 슬롯을 확보한다.
- 공용 벽 설비 슬롯은 재생성 호환용으로 유지하되 플레이 시 반복 모니터·소화기·비상전화는
  숨긴다. 화면 위쪽 벽은 문틀보다 높은 2.45m 입면으로 표시하고 방마다 저채도 벽지 패턴과
  시스템 패널 1개, 공간이 충분하면 CCTV·전자시계·상태계 중 1개를 추가한다.
- 중앙 가로·세로 2.2m 통행 축에는 충돌 가구를 두지 않는다. 큰 가구는 가장자리와
  코너에 배치하고, 문 안쪽 2.5m는 항상 비워 둔다.
- 생존자·빌런·해독제 장치 31개 위치에는 장치 외곽보다 0.15m 넓은 예약 구역을 두며,
  충돌 가구가 이 구역을 침범하면 씬 빌더 검증을 실패시킨다.
...
- 방 바닥은 공용 타일 색상 변형을 금지한다. 백신/실험/격리 A·B, 보관, 보안, 전력, 입원
  총 10개 방은 서로 다른 패널 형상·이음새 방향·표면 재질·주조색을 사용해 실루엣만으로도
  현재 구역을 구분할 수 있어야 한다.
...
- 단계가 올라도 전역 비상광은 실효 약 7%를 유지하며, 국소 조명과 표지 식별 범위만 추가한다.

### 9.1 0%

- 전역 비상광은 실효 약 7%로 유지해 벽 윤곽과 주요 동선만 겨우 읽을 수 있게 한다.
- 플레이어 개인등은 밝기 0.6%·외곽 반경 0.5m의 실루엣 확인용으로만 유지한다.
- 0% 단계에서 실제 지형의 세부 식별은 손전등의 54도 원뿔 내부로 제한한다.
...
- 현재 공간 이름은 월드 표지판 대신 우측 상단 HUD에 항상 표시
...
- 방 진입 후 우측 상단 HUD에서 현재 공간 이름을 1초 안에 식별할 수 있다.
